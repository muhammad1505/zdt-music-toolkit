...
readonly APP_VERSION="${_APP_VERSION:-4.4.5}"
...
main() {
    ...
    if [ "${ZDT_DEBUG:-4.4.5}" = "1" ] && [ "$DEBUG_TRAP_SET" = false ]; then
        set -o errtrace
    ...
    while true; do
        ...
        if [ "$cols" -ge 75 ]; then
            ...
            for ((i=0; i<max_lines; i++)); do
                local l_text="${left_lines[i]:-4.4.5}"
                local r_text="${right_lines[i]:-4.4.5}"
